...
struct tcp_hdr
{
    uint16_t   tcp_src_prt;	/* TCP source port */
    uint16_t   tcp_dst_prt;	/* Tcp destination port */
    uint32_t   tcp_seq_num;	/* TCP sequence number */
    uint32_t   tcp_ack_num;	/* acknowledgement number */
#if __BYTE_ORDER == __LITTLE_ENDIAN
    unsigned int tcp_rsrvd_1:4;
    unsigned int tcp_dt_ofst:4;
#elif __BYTE_ORDER == __BIG_ENDIAN
    unsigned int tcp_dt_ofst:4; //header length count of 4byte words
    unsigned int tcp_rsrvd_1:4;
#else
#error "Byte ordering not specified " 
#endif 
